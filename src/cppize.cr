...
*/
    ...
    def parse_and_transpile(code : String, filename : String = "<unknown>")
      begin
        ...
        puts ex.to_s(@current_filename,@show_trace)
        exit
      end
    end

    def parse_and_transpile(file : IO, filename : String = "<unknown>")
      parse_and_transpile file.gets_to_end, filename
    end
    ...
    class Error < ArgumentError
      ...
      property cause_node : ASTNode?
      @catched = false
      @cause_node = Nop.new

      protected def location_to_s(filename : String, location : Location?)
        if location.nil?
          "file #{filename} ; location unknown"
        else
          "file #{filename} ; location line #{location.line_number} col #{location.column_number}"
        end
      end

      def initialize(m,cn : ASTNode? = nil)
        super m
        @cause_node = cn.as(ASTNode) unless cn.nil?
      end

      def to_s(fname : String,trace : Bool = false)
        str = ""
        if cause_node
          c = cause_node.not_nil!
          str = "#{message}\nCaused by #{c.class.name}\nBegin : #{location_to_s fname, c.location}\nEnd : #{location_to_s fname,c.end_location}"
        else
          str = message.to_s
        end
    ...
    protected def transpile(node, should_return : Bool = false)
      if @failsafe
        ...
        raise Error.new("Node type #{node.class} isn't supported yet",node || Nop.new)
      end
    end
